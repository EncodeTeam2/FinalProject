...
import StyledBoard, { StyledSnake, StyledAppearFood, StyledSpinFood, StyledStaticFood } from "./style";
...
const Board = ({ cellCount, food, snake, state, debug }: BoardProps) => {
  const opacity = state === GameState.End || state === GameState.Pausing ? "0.3" : "0.75";
  ...
  const selectFood = (key: number) => {
    switch (state) {
      ...
      default:
        return <StyledAppearFood key={key} $opacity={opacity} $state={animationState} />;
    }
  };
